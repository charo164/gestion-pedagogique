...
@Component({
  selector: 'app-attache-session-course-detail',
  templateUrl: './attache-session-course-detail.component.html',
  styleUrls: ['./attache-session-course-detail.component.css'],
})
export class AttacheSessionCourseDetailComponent implements OnInit {
  ...
  refreshSession(id: number) {
    this.sessionCourseService
      .update(id.toString(), { updated_at: new Date().toLocaleString() })
      ?.subscribe((res) => {
        if ('error' in res) return;
